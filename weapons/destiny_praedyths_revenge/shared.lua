...
SWEP.PrintName						= "Praedyth's Revenge"
SWEP.Type							= "Praedyth's fall isn't over… because it hasn't happened yet… and it will happen again."
SWEP.DrawAmmo						= true
SWEP.data 							= {}
...
SWEP.Primary.RPM_Displayed			= 140
...
SWEP.Primary.Sound 					= Sound ("TFA_PRAEDYTHS_FIRE.1")
SWEP.Primary.ReloadSound 			= Sound ("TFA_PRAEDYTHS_RELOAD.1")
...
SWEP.ViewModel						= "models/weapons/PraedythsRevenge/c_Praedyths_Revenge.mdl"
SWEP.WorldModel						= nil
...
SWEP.VMPos = Vector(-.6, -1.2, .5)
SWEP.CrouchPos = Vector(-.6, -1.2, .5)
SWEP.VMAng = Vector(.3, .5, -.5)
SWEP.VMPos_Additive = false

SWEP.IronSightsPos = Vector(-5.7725, -12.6, .275)
...
SWEP.VElements = {
	["scope"] = { type = "Model", model = "models/rtcircle.mdl", bone = "WeaponBone", rel = "", pos = Vector(0, -3.89, .775), angle = Angle(0, -90, 180), size = Vector(0.295, 0.295, 0.295), color = Color(255, 255, 255, 255), surpresslightning = false, material = "!tfa_rtmaterial", skin = 0, bodygroup = {} }
}


SWEP.WElements = {
	["world"] = { type = "Model", model = SWEP.ViewModel, bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-13, 6.699, -5.715), angle = Angle(-8.183, 0, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.RTReticleMaterial = Material("models/PraedythsRevenge/PraedythsRevengeReticle")
SWEP.Compass = Material("models/PraedythsRevenge/PraedythsRevengeCompass")
...
function SWEP:Think2(...)
	if CLIENT then
		if self:GetIronSights() then
			local fadein = self:GetIronSightsProgress()
			self.VElements["scope"].color = Color(255, 255, 255, (255*self:GetIronSightsProgress()))
			self.Compass:SetVector("$color2", Vector(fadein, fadein/2, 0))
		else
			self.VElements["scope"].color = Color(255, 255, 255, 0)
			self.Compass:SetVector("$color2", Vector(0,0,0))
		end
	end

...
SWEP.PrintName				= "Divine Blaster"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Divine_Blaster.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("Divine_Blaster.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 80					-- Damage, in standard damage points.
SWEP.Primary.Damage		= 150				-- Damage, in standard damage points.
SWEP.Primary.Damage_Secondary		= 350					-- Damage, in standard damage points.
SWEP.Primary.BlastRadius	= 150               -- Radius for DMG_BLAST
SWEP.Primary.DamageType = DMG_BULLET --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.
...
SWEP.Primary.RPM_Semi				= 250					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
SWEP.FiresUnderwater = true

-- nZombies Stuff
SWEP.NZWonderWeapon		= false	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
--SWEP.NZRePaPText		= "your text here"	-- When RePaPing, what should be shown? Example: Press E to your text here for 2000 points.
SWEP.NZPaPName				= "More Broken Guns"
...
SWEP.Primary.Spread		= .036					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_divine_blaster.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_divine_blaster.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(8.399, 0, 1.519)
SWEP.IronSightsAng = Vector(0.986, 1.164, 5.264)
...
SWEP.SmokeParticles = { ar2 = "smoke_trail_tfa",  --These are particle effects INSIDE a pcf file, not PCF files, that are played when you shoot.
	smg = "smoke_trail_tfa",
...
SWEP.MuzzleFlashEffect = "cso_muz_winggun" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.MuzzleFlashEffectSilenced = "cso_muz_winggun"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_winggun"	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_divine_blaster")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.TracerName = "cso_tra_winggun"
			self.Primary_TFA.DamageType = DMG_BULLET
			self.Primary_TFA.RPM = 250
			self.Primary_TFA.Spread = 0.036
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 8
			self.MuzzleFlashEffect = "cso_muz_winggun"
			self:StopSound("Divine_Blaster.Loop")
			self.Primary_TFA.ImpactEffect = nil
			self:ClearStatCache()
		else
			self.TracerName = "tra_winggun"
			self.Primary_TFA.DamageType = bit.bor(DMG_BLAST,DMG_ALWAYSGIB)
			self.Primary_TFA.Damage = self.Primary_TFA.Damage_Secondary
			self.Primary_TFA.RPM = 30
			self.Primary_TFA.Spread = 0.005
			self.Primary_TFA.AmmoConsumption = 10
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "cso_muz_winggun"
			self.Primary_TFA.ImpactEffect = "exp_divine_blaster"
			self:ClearStatCache()
		end
	end

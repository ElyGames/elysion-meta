...
SWEP.PrintName						= "Piece Of Mind"
SWEP.Type							= "With words or weapons, inaccuracy can be fatal."
SWEP.DrawAmmo						= true
SWEP.data 							= {}
...
SWEP.Primary.Sound 					= Sound ("TFA_PIECEOFMIND_FIRE.1");
SWEP.Primary.ReloadSound 			= Sound ("TFA_PIECEOFMIND_RELOAD.1");
...
SWEP.Primary.RPM_Burst = 1080 -- RPM for burst fire, overrides semi.  This is in Rounds Per Minute / RPM
...
SWEP.ViewModel						= "models/weapons/pieceofmind/c_piece_of_mind.mdl"
...
SWEP.Primary.KickHorizontal			= 0.11
...
SWEP.IronSightsPos = Vector(-4.6885,-6.5, 1.48) 
...
SWEP.VElements = {
	//["reticle"] = { type = "Model", model = "models/hunter/plates/plate1x1.mdl", bone = "Reticle", rel = "", pos = Vector(0,-8,1), angle = Angle(0, 0, -90), size = Vector(.005, .005, 0), color = Color(255, 255, 255, 255), surpresslightning = false, material = "reticle/destiny2_reddot", skin = 0, bodygroup = {} },
	["scope"] = { type = "Model", model = "models/rtcircle.mdl", bone = "Reticle", rel = "", pos = Vector(-.005,2.15,1.03), angle = Angle(0, -90, 180), size = Vector(.4, .4, .4), color = Color(255, 255, 255, 255), surpresslightning = false, material = "!tfa_rtmaterial", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["world"] = { type = "Model", model = SWEP.ViewModel, bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-11, 6.752, -6.1), angle = Angle(-8.183, 1.169, 180), size = Vector(0.9, 0.9, 0.9), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.EventTable = {
	
	[ACT_VM_DRAW] = {															
		{ ["time"] = 0, ["type"] = "sound", ["value"] = Sound("TFA_PIECEOFMIND_DRAW.1") }
	}
}
...
SWEP.ViewModelPunch_MaxVertialOffset = 1.25 -- Default value is 3
...
SWEP.RTReticleMaterial = Material("models/pieceofmind/pieceofmindReticle")
SWEP.RTReticleScale = 1.11
SWEP.RTShadowScale = 2.3
...
SWEP.RTScopeFOV = 90 / 2.25 / 2 -- Default FOV / Scope Zoom / screenscale
...
function SWEP:Think(...)
	if CLIENT then
		if self:GetIronSightsProgress() > 0.75 then
			self.VElements["scope"].color = Color(255, 255, 255, self:GetIronSightsProgress()*255)
		else

...
SWEP.PrintName				= "CHARGER-5"		-- Weapon name (Shown on HUD)	
...
SWEP.Type	= "Rare Grade Assault Rifle"
SWEP.Primary.Knockback = 0 
--[[WEAPON HANDLING]]--

--Firing related
SWEP.Primary.Sound 			= Sound("Charger5.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "Duracell Bullpup"
--SWEP.NZPaPReplacement 	= "tfa_cso_expert"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
SWEP.Primary.Spread		= .00665					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy = .003325	-- Ironsight accuracy, should be the same for shotguns
...
SWEP.IronSightsMoveSpeed = 0.78 --Multiply the player's movespeed by this when sighting.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_charger5.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_charger5.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(5.79, 0, 0.972)
SWEP.IronSightsAng = Vector(1.108, 0, 0)
...
SWEP.Secondary.Damage = 250 --Damage
SWEP.Secondary.Sound = Sound( "CHARGER7.FireLaser" )
SWEP.Secondary.ClipSize = -1
SWEP.Secondary.DefaultClip = 0
SWEP.Secondary.Automatic = true
SWEP.Secondary.Ent = nil
...
function SWEP:SecondaryAttack()
    ...
    local Seq = VModel:LookupSequence("shoot_laser2")
    if self:Ammo2() % 1 == 0 then
    ...
    if SERVER then
        ...
        bullet.TracerName = "cso_tra_charger7"
    ...
    self:EmitSound("CHARGER7.FireLaser")
    ...
    self:SetNextIdleAnim(CurTime() + 1.14)
end
...
function SWEP:PostPrimaryAttack()
	...
	if SERVER then
		if self.NextGen >= 5 and self:Ammo2() < 20 then
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_charger5")

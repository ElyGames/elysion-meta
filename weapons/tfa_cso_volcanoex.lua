...
SWEP.PrintName				= "Inferno Cannon"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Volcanoex.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Secondary.Ent = "gatlingex_fireball"

SWEP.Secondary.Model = "models/hunter/misc/sphere1x1.mdl" --Projectile Model
SWEP.Secondary.Damage = 2000 --Grenade Damage
SWEP.Secondary.Velocity = 300 --Grenade Velocity
SWEP.Secondary.Delay = 1 --Delay in seconds
SWEP.Secondary.Sound = Sound("Volcanoex.Fire2") --Firing sound
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_volcanoex.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_volcanoex.mdl" -- same case UHGI;DFSD/
...
SWEP.IronSightsPos = Vector(14.269, -2.512, 2.408)
SWEP.IronSightsAng = Vector(0, 10.958, -3.142)
...
function SWEP:Think()
    if SERVER then
        if self.NextGen <= CurTime() and self:Ammo2() < 3 then
        end
    end
...
function SWEP:PostPrimaryAttack()
    if SERVER then
        ...
        if self.NextGen >= 15 and self:Ammo2() < 3 then
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_volcanoex")

...
SWEP.PrintName				= "MP7A1"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("MP7A1.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("MP7A1.Fire") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 29					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 36					-- Damage, in standard damage points.
...
--SWEP.NZPaPName				= "Useless SMG"
SWEP.NZPaPReplacement 	= "tfa_cso_mp7a160r"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
--Recoil Related
SWEP.Primary.KickUp			= 0.14				-- This is the maximum upwards recoil (rise)
...
SWEP.Primary.StaticRecoilFactor = 0.24 	--Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.

--Firing Cone Related

SWEP.Primary.Spread		= .007					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_mp7a1.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_mp7a1.mdl" -- Worldmodel path
...
SWEP.RunSightsPos = Vector(0, -15, -10)
...
SWEP.IronSightsPos = Vector(8.432, -8, -1)
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_mp7a1")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Primary_TFA.Spread		= .007
            self.Primary_TFA.IronAccuracy = .0053	
            ...
            self.Primary_TFA.KickDown	= 0.08		
            self.Primary_TFA.KickHorizontal	= 0.10	
			self.Primary_TFA.Spread = .005
			self.Primary_TFA.IronAccuracy = .0025

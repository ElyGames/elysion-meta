...
SWEP.PrintName				= "M2 Devastator"		-- Weapon name (Shown on HUD)
...
SWEP.ViewModelPunchYawMultiplier_IronSights = 0 -- Default value is 1.5

--Firing related  
SWEP.Primary.Sound 			= Sound("M2Devastator.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage		= 205					-- Damage, in standard damage points.
SWEP.DamageType = bit.bor(DMG_BULLET,DMG_AIRBOAT,DMG_BLAST,DMG_ALWAYSGIB) --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.
...
SWEP.Primary.BlastRadius	= 20               -- Radius for DMG_BLAST

SWEP.Secondary.ClipSize = -1
SWEP.Secondary.DefaultClip = 0
SWEP.Secondary.Ammo = "m2_missile"
SWEP.Secondary.Ent = "mgsm_missile"

SWEP.Secondary.Model = "models/weapons/tfa_cso/w_mgsm_missile.mdl" --Projectile Model
SWEP.Secondary.Damage = 50 --Grenade Damage
SWEP.Secondary.Velocity = 7500 --Grenade Velocity
SWEP.Secondary.Delay = 0 --Delay in seconds
SWEP.Secondary.Sound = Sound("M2Devastator.Fire2") --Firing sound
SWEP.Secondary.Recoil = Angle(0,0,0) --Recoil Dir

-- nZombies Stuff
SWEP.NZWonderWeapon		= false	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
--SWEP.NZRePaPText		= "your text here"	-- When RePaPing, what should be shown? Example: Press E to your text here for 2000 points.
SWEP.NZPaPName				= "Rocket 48"
...
SWEP.Primary.ClipSize			= 350					-- This is the size of a clip
SWEP.Primary.DefaultClip			= 1050				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.Primary.StaticRecoilFactor = 0.22 	--Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_m2_devastator.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_m2_devastator.mdl" -- same case UHGI;DFSD/
...
SWEP.WElements = {
    ["backpack_launcher"] = { type = "Model", model = "models/weapons/tfa_cso/w_mgsmlauncher.mdl", bone = "ValveBiped.Bip01_Spine2", rel = "", pos = Vector(10, 6, 4.5), angle = Angle(90, -10, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.ProceduralHolsterPos = Vector(3, 0, -15)
SWEP.ProceduralHolsterAng = Vector(-10, 30, 10)
...
SWEP.MuzzleFlashEffect = "mgsm_muz" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "mgsm_tra" 	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Primary.ImpactEffect = "exp_mgsm2"
...
function SWEP:SecondaryAttack()
	if CurTime() >  self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() > 0 then
		...
		self.Secondary.Nextregen =  CurTime() + 10
		if SERVER then
...
if self.Secondary.Nextregen <= CurTime()  and self:Ammo2() < 1 then 
self.Owner:GiveAmmo( 1,"m2_missile", true )                     -------SECONDARY AMMO REGEN
self.Secondary.Nextregen =  CurTime()  + 6   ---3 seconds
self:EmitSound("M2Devastator.Launcher_Ready")
end
end
...
if SERVER then
self.Secondary.Nextregen = CurTime() + 5
end
return BaseClass.Deploy(self,...)
end

if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_m2_devastator")

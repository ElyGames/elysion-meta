...
SWEP.PrintName				= "SPAS-12 Deluxe (A Mode)"		-- Weapon name (Shown on HUD)
SWEP.NZPaPName				= ""			-- For nZombies. Pack-a-punch this gun to get this name!
...
--Firing related
SWEP.Primary.Sound 			= Sound("Spas12.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("Spas12.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 15					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 12					-- Damage, in standard damage points.
...
SWEP.Primary.DefaultClip			= 40				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.Primary.StaticRecoilFactor = 0.30 	--Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_spas12ex.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_spas12ex.mdl" -- Worldmodel path
...
SWEP.BoltTimerOffset = 0.05 --How long you stay sighted in after shooting, with a bolt action.
...
SWEP.IronSightsPos = Vector(7.519, 0, -0.634)
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_spas12ex")
...
--function SWEP:Initialize()
	...
	--self.StatCache_Blacklist["Primary.Automatic"] = true
	...
	--self.StatCache_Blacklist["BaseAnimations"] = true
...
--function SWEP:Think2(...)
	--if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		--if self:GetSilenced() then
			...
			--self.Primary_TFA.KickUp	= 0.16	
            --self.Primary_TFA.KickDown = 0.10				
            --self.Primary_TFA.KickHorizontal	= 0.10
			--self.Primary_TFA.Spread		= .04
            --self.Primary_TFA.IronAccuracy = .04
		...
		--else
			...
			--self.Primary_TFA.AmmoConsumption = 1
			--self.Primary_TFA.NumShots = 8
			--self.MuzzleFlashEffectSilenced = "tfa_muzzleflash_shotgun"
			--self.Primary_TFA.ClipSize = 8
			--self.Primary_TFA.KickUp	= 0.15	
            --self.Primary_TFA.KickDown	= 0.10		
            --self.Primary_TFA.KickHorizontal	= 0.10	
			--self.Primary_TFA.Spread = .030
			--self.Primary_TFA.IronAccuracy = .030

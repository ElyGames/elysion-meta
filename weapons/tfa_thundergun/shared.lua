...
SWEP.Purpose				= "Storm into hordes of zombies with this big thundering cannon of death!" --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.DrawCrosshairIS = false --Draw the crosshair in ironsights?
SWEP.PrintName				= "Thundergun"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound( "Weapon_Thundergun.Shoot" ) -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.DefaultClip = 14 -- This is the number of bullets the gun gives you, counting a clip as defined directly above.
SWEP.Primary.Ammo = "Thumper" -- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
...
--Firing Cone Related
SWEP.Primary.Spread = 0.018--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
...
--[[PROJECTILES]]--
--SWEP.ProjectileEntity = "obj_thundergun_proj" --Entity to shoot
SWEP.ProjectileVelocity = 0 --Entity to shoot's velocity
SWEP.ProjectileModel = nil --Entity to shoot's model
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/thundergun/v_thundergun.mdl" --Viewmodel path
SWEP.ViewModelFOV			= 65		-- This controls how big the viewmodel looks.  Less is more.
SWEP.ViewModelFlip			= false		-- Set this to true for CSS models, or false for everything else (with a righthanded viewmodel.)
SWEP.UseHands = true --Use gmod c_arms system.

SWEP.VMPos = Vector(0, 0, -1.609)
...
--SWEP.VMPos = Vector(-1, -0.603, -1.005) 
	--[0] = 1,
	--[1] = 4,
	--[2] = etc.
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/thundergun/w_thundergun.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-4.321, 3.216, 0.92)
SWEP.IronSightsAng = Vector(0, 0.703, 0)
--[[INSPECTION]]--
...
function SWEP:PrimaryAttack(old)
	...
	local ppos = pos + (own:GetAimVector() * 250)
	
	
	if self:Clip1() > 1 then
		self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
		self:EmitSound("weapons/thundergun/fly_thundergun_shoot.ogg")
	else
		self:SendWeaponAnim(ACT_VM_PRIMARYATTACK_EMPTY)
		self:EmitSound("weapons/thundergun/fly_thundergun_last_shoot.ogg")
	end
	
	
	if SERVER then
		local pos = self.Owner:GetShootPos() + (own:GetForward() * 60)
			local orb1 = ents.Create("obj_thundergun_proj")
			orb1:SetPos(pos)
			orb1:SetAngles(own:GetAngles())
			orb1:Spawn()
			orb1.Owner = own
		ParticleEffect( "thundergun_clouds2", own:GetPos()+Vector(0,0,32), Angle(0,own:GetAngles().y,0) )
	end
	
	local ed = EffectData()
	ed:SetOrigin(pos)
	ed:SetStart(pos + own:GetForward() * 10)
	ed:SetAttachment(1)
	ed:SetEntity(self)
	util.Effect( "effect_dist", ed )
...
function SWEP:DoShot(ppos)
	...
	if SERVER then
		for k,v in pairs(ents.FindInSphere(ppos,250)) do
			...
			if gamemode.Get("nzombies") and v:IsPlayer() and hook.Run("PlayerShouldTakeDamage", v, own) then 
			   return 
			end	
...
function SWEP:Reload(test)
	if self:Ammo1() < 1 or self:Clip1()>0 then 
		return
	end
	BaseClass.Reload( self, test ) 
end
...
function SWEP:PreDrawViewModel( vm )
	...
	local lights = {8, 9, 10, 7}
	local lights1 = {7, 10, 9, 8}
	
	for i = 1, #lights - self:Clip1() do
		vm:SetSubMaterial(lights1[i], "models/weapons/thundergun/_-gt5_weapon_mesh_gray.vmt")
	end
	for i = 1, self:Clip1() do
		vm:SetSubMaterial(lights[i], nil)
	end
end
...
SWEP.Purpose				= "Right click or suit zoom (X by default) to change mode" --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "SKULL-2"		-- Weapon name (Shown on HUD)
...
SWEP.Revolver			= true			-- New TFA Base feature. Tell it that I'm a revolver, not a bolt-action.
...
--Firing related
SWEP.Primary.Sound 			= Sound("SKULL2.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("SKULL2.Fire") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 75					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 75					-- Damage, in standard damage points.
...
SWEP.Primary.RPM_Semi				= 480					-- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
SWEP.FiresUnderwater = false

-- nZombies Stuff
SWEP.NZWonderWeapon		= false	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
--SWEP.NZRePaPText		= "your text here"	-- When RePaPing, what should be shown? Example: Press E to your text here for 2000 points.
SWEP.NZPaPName				= "CROSSBONES-4"
...
SWEP.Primary.DefaultClip			= 154				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.Primary.Spread		= .0084					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy = .0042	-- Ironsight accuracy, should be the same for shotguns
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_skull_2.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_skull2_b.mdl" -- Worldmodel path
...
SWEP.WElements = {
	["duel1+"] = { type = "Model", model = "models/weapons/tfa_cso/w_skull2.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(7, 1, 3.25), angle = Angle(0, 80, 0), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_skull2")
...
function SWEP:Initialize()
	...
	self.StatCache_Blacklist["WorldModel"] = true
    self.StatCache_Blacklist["Offset"] = true
	self.StatCache_Blacklist["HoldType"] = true
	self.StatCache_Blacklist["WElements"] = true
	self.StatCache_Blacklist["Primary.ClipSize"] = true
	self.StatCache_Blacklist["Akimbo"] = true
	self.StatCache_Blacklist["RunSightsPos"] = true
	self.StatCache_Blacklist["RunSightsAng"] = true
	self.StatCache_Blacklist["InspectPos"] = true
	self.StatCache_Blacklist["InspectAng"] = true
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
            ...
            self.Primary_TFA.KickDown = 0.15				
            self.Primary_TFA.KickHorizontal	= 0.2
			self.Primary_TFA.Spread		= .0084
            self.Primary_TFA.IronAccuracy = .0042
			self.WorldModel			= "models/weapons/tfa_cso/w_skull2.mdl"
			self.HoldType = "duel"
			self.Offset = {
			Pos = {
			Up = -3.25,
			...
			self.Akimbo = true
			self.RunSightsPos = Vector(0, 0, 0)
			self.RunSightsAng = Vector(-16.223, -0.562, 0)
			self.InspectPos = Vector (0,0,0)
			self.InspectAng = Vector (0,0,0)
			self:Unload()
		    self:CompleteReload()
		    self:ClearStatCache()
		else
			...
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "tfa_muzzleflash_revolver"
			...
			self.Primary_TFA.Spread = .0035
			self.Primary_TFA.IronAccuracy = .00175
			self.WorldModel			= "models/weapons/tfa_cso/w_skull2.mdl"
			...
			self.Akimbo = false
			self.RunSightsPos = Vector(0,-5.5,-4)
			self.RunSightsAng = Vector(42.5,0,0)
			self.WElements = nil
			self.InspectPos = nil
			self.InspectAng = nil
			self:Unload()

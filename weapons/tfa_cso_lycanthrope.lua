...
SWEP.PrintName				= "SG552 Lycanthrope"		-- Weapon name (Shown on HUD)
...
SWEP.NZPaPReplacement 	= "tfa_cso_lycanthrope_expert"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
SWEP.Primary.KickDown			= 0.185					-- This is the maximum downwards recoil (skeet)
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_lycanthrope.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_lycanthrope.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_lyc" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_lyc" 	--Change to a string of your tracer name.  Can be custom.
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_lycanthrope")
...
function SWEP:Think2(...)
	if self:GetIronSightsProgress() <= 0.5 and self:GetCSO_SwitchState() then
        ...
        self.Primary_TFA.KickDown	= 0.185		
        self.Primary_TFA.KickHorizontal	= 0.225	
	...
	elseif self:GetIronSightsProgress() > 0.5 and not self:GetCSO_SwitchState() then
        ...
        self.Primary_TFA.KickDown	= 0.09		
        self.Primary_TFA.KickHorizontal	= 0.1125	
		self.MuzzleFlashEffect = "cso_muz_lyc"

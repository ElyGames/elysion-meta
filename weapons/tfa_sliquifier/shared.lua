...
SWEP.Contact				= ""
SWEP.Purpose				= "Made in China (literally), this post-apocalyptic handcrafted thing shoots splippery ooze that boils zombies from inside!"
SWEP.Instructions				= ""
...
SWEP.Primary.Sound			= Sound("Weapon_Slipgun.Shoot")		-- Script that calls the primary fire sound
SWEP.Primary.RPM			= 81			-- This is in Rounds Per Minute
...
SWEP.Primary.Damage		= 5500000	-- Base damage per bullet
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if self:GetIronSights() then
			pos = own:GetShootPos() + own:GetUp()*-4 + own:GetForward()*40
		else
			pos = own:GetShootPos() + own:GetUp()*-5 + own:GetRight()*8 + own:GetForward()*40
		end
		--debugoverlay.Sphere(pos,5) 
		orb1.Packed = self.Ispackapunched
		orb1:SetPos(pos)
	...
	self.Weapon:EmitSound("Weapon_Slipgun.Shoot")
	self.Owner:ViewPunch( Angle(-2, 0 ))
end

-- Nzombies stuff
SWEP.NZWonderWeapon		= true	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
SWEP.NZPaPName				= "G.E.S Oozinator"
SWEP.DisableChambering = true
...
function SWEP:PreDrawViewModel( vm )
	if self.Ispackapunched then
		...
		vm:SetSubMaterial(1, "models/weapons/slipgun/zmb_slipgun_scroll_p")
	else

...
SWEP.PrintName				= "Eradicator Railgun"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModel				= "models/weapons/tfa_qc/c_railgun_eradicator.mdl"	-- Weapon view model
SWEP.WorldModel				= "models/weapons/tfa_qc/w_railgun_eradicator.mdl"	-- Weapon world model
SWEP.UseHands = true
...
SWEP.Primary.Sound			= Sound("TFA_Eradicator.1")		-- script that calls the primary fire sound
...
SWEP.Primary.Damage		= 1600	--base damage per bullet
...
SWEP.NZPaPReplacement 	= "tfa_qc_eradicator_pap"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
SWEP.TracerName = "tfa_tracer_eradicator"
SWEP.TracerCount = 1
SWEP.ImpactDecal = "FadingScorch"--"Scorch"

SWEP.MuzzleFlashEffect = "tfa_muzzleflash_eradicator"
...
function SWEP:Think( ... )
	if self:GetNW2Bool("Charging", true) and self.railsound then 
		self.railsound:Stop()
	else
		self.railsound = CreateSound(self.Weapon, "weapons/qcrailgun/railgun-idle.ogg")
		self.railsound:Play()
	end
...
function SWEP:Deploy( ... )
	if (self.Owner:Alive() or IsValid(self.Owner) and IsValid(self.Weapon)) and not self.railsound then
		self.railsound = CreateSound(self.Weapon, "weapons/qcrailgun/railgun-idle.ogg")
		self.railsound:Play()
	end
...
function SWEP:Holster( ... )
	if (self.Owner:Alive() or IsValid(self.Owner) or not IsValid(self.Weapon)) and self.railsound then self.railsound:Stop() end
	return BaseClass.Holster(self,...)
end

function SWEP:OnRemove( ... )
	if self.railsound then 
		self.railsound:Stop()
	end

...
SWEP.PrintName				= "Lightning Bazzi-1"		-- Weapon name (Shown on HUD)	
...
--Firing related
SWEP.Primary.Sound 			= Sound("CartRed.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("CartRed.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 29					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 35					-- Damage, in standard damage points.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_cartred.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_cartred.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(9.034, -7.796, -0.585)
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_cartred")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
            ...
            self.Primary_TFA.KickHorizontal	= 0.12	
			self.data.ironsights = 0
			self.WorldModel			= "models/weapons/tfa_cso/w_cartred.mdl"
			self.Offset = {
			Pos = {
			Up = -6,
			Right = 1,
			...
			self.Primary_TFA.Spread		= .008
            self.Primary_TFA.IronAccuracy = .004
		else
			self.TracerName = nil
			self.Primary_TFA.Damage = self.Primary_TFA.Damage_Secondary
			self.Primary_TFA.RPM = 550
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			...
			self.Primary_TFA.Spread		= .0075
            self.Primary_TFA.IronAccuracy = .00375
		end
	end

...
SWEP.Purpose				= "Press suit zoom (X by default) to change mode" --Purpose Tooltip
...
--Firing related
SWEP.Primary.Sound 			= Sound("XM8.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("XM8.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 29					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular
SWEP.Primary.Damage_Secondary		= 60					-- Damage, in standard damage points.
...
SWEP.NZPaPName				= "XtraMassacre 16"
...
SWEP.Primary.Spread		= .0055					--This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy = .00275	-- Ironsight accuracy, should be the same for shotguns
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_xm8s.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_xm8.mdl" -- Worldmodel path
...
SWEP.IronSightsPos = Vector(6.95, -2.368, 1.103)
SWEP.IronSightsAng = Vector(-1, -5.0, 8.845)
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_xm8")
...
function SWEP:Initialize()
	...
	self.StatCache_Blacklist["Secondary.IronFOV"] = true
	self.StatCache_Blacklist["Secondary.ScopeTable"] = true
	self.StatCache_Blacklist["Secondary.UseACOG"] = true
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.Secondary_TFA.IronFOV	= 40
			self.Secondary_TFA.ScopeTable = nil
			self.Secondary_TFA.UseACOG = true
			self:ClearStatCache("Secondary.ScopeTable")
		else
			self.TracerName = nil
			...
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "tfa_muzzleflash_rifle"
			self.Primary_TFA.KickUp	= 0.14	
            self.Primary_TFA.KickDown	= 0.10		
            self.Primary_TFA.KickHorizontal	= 0.14	
			self.Primary_TFA.StaticRecoilFactor = 0.4
			self.Primary_TFA.Spread = .004
			self.Primary_TFA.IronAccuracy = .002
			self.WorldModel			= "models/weapons/tfa_cso/w_xm8sharpshooter.mdl"
			self.Offset = {
			Pos = {
			Up = -5.2,
			Right = 0.5,
			...
			self.Secondary_TFA.IronFOV	= 20
			self.Secondary_TFA.ScopeTable = {
			...
			self.Secondary_TFA.UseACOG = false
			self:ClearStatCache("Secondary.ScopeTable")
		end
	end

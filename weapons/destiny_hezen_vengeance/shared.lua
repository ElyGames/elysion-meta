...
SWEP.PrintName						= "Hezen Vengeance"
SWEP.Type							= "The Hezen Axis Mind is tireless and infinite. While it cannot be stopped… it can be paused."
SWEP.DrawAmmo						= true
SWEP.data 							= {}
...
SWEP.Primary.Sound 					= Sound ("TFA_HEZENV_FIRE.1")
SWEP.Primary.ReloadSound 			= Sound ("TFA_HEZENV_RELOAD.1")
...
-- PROJECTILES
SWEP.Primary.Projectile = "destiny_normal_rocket" -- Entity to shoot
...
SWEP.ViewModel						= "models/weapons/HezenVengeance/c_Hezen_Vengeance.mdl"
...
SWEP.IronSightsPos = Vector(-3.265, -9.5, .887)
...
SWEP.Attachments = {
	[1] = { offset = { 0, 0 }, atts = { "d2mp_backup_mag", "d2mp_sprint_grip", }, order = 3 },
	[2] = { offset = { 0, 0 }, atts = { "d2mp_quick_access_sling", "d2mp_counterbalance" }, order = 2 },
	[3] = { offset = { 0, 0 }, atts = { "d2mp_dragonfly_spec", "d2mp_reconstruction" }, order = 3 },
}
...
SWEP.RTReticleMaterial = Material("models/HezenVengeance/HezenVengeanceReticle")
//SWEP.RTShadowMaterial = Material("models/tataragaze/TataraGazeReticleShadow")
SWEP.RTReticleScale = 1.04
SWEP.RTDotScale = .05
SWEP.RTShadowScale = 0
SWEP.RTMaterialOverride = 4 -- Take the material you want out of print(LocalPlayer():GetViewModel():GetMaterials()), subtract 1 from its index, and set it to this.
SWEP.RTOpaque = true -- Do you want your render target to be opaque?
SWEP.RTScopeFOV = 90 / 1.1 / 2 -- Default FOV / Scope Zoom / screenscale
...
function SWEP:PrimaryAttack(...)
	if self:CanPrimaryAttack(true) then
		ParticleEffectAttach("destiny_rocket_muzzle", 4, self.OwnerViewModel, self.OwnerViewModel:LookupAttachment("muzzle"))
	end

...
SWEP.Category				= "TFA SCP" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
SWEP.Manufacturer = "" --Gun Manufactrer (e.g. Hoeckler and Koch )
SWEP.Author				= "Hosti & Gmodlife" --Author Tooltip
...
SWEP.PrintName				= "L'arme vivante - SCP-127-"..table.Random( alphabet )		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]-- 
SWEP.Primary.Sound = Sound("TFA_INS2.MP5K.1") -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("TFA_INS2.MP5K.2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.PenetrationMultiplier = 1 --Change the amount of something this gun can penetrate through
SWEP.Primary.Damage = 85 -- Damage, in standard damage points. 
SWEP.Primary.DamageTypeHandled = true --true will handle damagetype in base  
SWEP.Primary.DamageType = bit.bor(DMG_PLASMA, DMG_NERVEGAS, DMG_POISON)  --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.  Leave nil to autodetect.  DMG_AIRBOAT opens doors.
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_scp127/c_scp127.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_scp127/w_scp127.mdl" -- Weapon world model path
...
SWEP.Secondary.IronFOV_Kobra = 63

SWEP.IronSightsPos_EOTech = Vector(-3.03, -4, -0.01)
SWEP.IronSightsAng_EOTech = Vector(0, -0.165, 0)
SWEP.Secondary.IronFOV_EOTech = 64

SWEP.IronSightsPos_RDS = Vector(-3.03, -4, -0.01)
SWEP.IronSightsAng_RDS = Vector(0, -0.165, 0)
SWEP.Secondary.IronFOV_RDS = 60

SWEP.IronSightsPos_2XRDS = Vector(-3.015, -4, -0.014)
SWEP.IronSightsAng_2XRDS = Vector(0, 0, 0)
SWEP.Secondary.IronFOV_2XRDS = 56
SWEP.ScopeAngleTransforms_2XRDS = Angle(0, -0.175, 0)
SWEP.RTOverlayTransforms_2XRDS = { 0.0175, 0 }

SWEP.IronSightsPos_C79 = Vector(-3.016, -4, -0.368)
SWEP.IronSightsAng_C79 = Vector(0, 0, 0)
SWEP.Secondary.IronFOV_C79 = 60
SWEP.ScopeAngleTransforms_C79 = Angle(0, -0.175, 0)
SWEP.RTOverlayTransforms_C79 = { 0.0175, 0 }

SWEP.IronSightsPos_PO4X = Vector(-2.954, -4, 0.097)
SWEP.IronSightsAng_PO4X = Vector(0, 0, 0)
SWEP.Secondary.IronFOV_PO4X = 52
SWEP.ScopeAngleTransforms_PO4X = Angle(0, -0.175, 0)
SWEP.RTOverlayTransforms_PO4X = { 0.0175, 0.025 }
...
hook.Add('EntityTakeDamage', 'scp127_gun_poison', function(ent, dmg)
	...
	if not IsValid(wep) or wep:GetClass() ~= 'tfa_scp_scp127' then
		return
	end
	
	local id = 'scp127_timer' .. ent:SteamID64()
	if timer.Exists(id) then
		return
	end

	local damage = 8
	timer.Create(id, 1, 25, function()
		local oldHealth = ent:Health()

		-- leave player with 1hp
		if oldHealth - damage <= 1 then
			timer.Remove(id)
			ent:SetHealth(1)
		...
		ent:TakeDamage(8, att, att)

		-- just in case lol
		if ent:Health() == oldHealth then
			ent:TakeDamage(8)
		end
	end)
end)

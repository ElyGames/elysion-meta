...
SWEP.Purpose				= "testing new alt attack coding" --Purpose Tooltip
...
SWEP.PrintName				= "Tiamat MK-3 Proto"		-- Weapon name (Shown on HUD)
...
function SWEP:ChooseShootAnim(...)
	local c = self:GetCharge()
	if c > 1 then
		local typev, tanim = AnimTable["charge_shoot"].type, AnimTable["charge_shoot"].value
		if typev ~= TFA.Enum.ANIMATION_SEQ then
			return self:SendViewModelAnim(tanim)
		else
			return self:SendViewModelSeq(tanim)
...
function SWEP:Think2(...)
	...
	local c = self:GetCharge()
	self.Primary.Damage = self.Primary.DamageChargeTable[math.Clamp(c, 0, 1)]
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_plasmagunexc_proto")

...
SWEP.Purpose				= "Press suit zoom (X by default) to fire alt attack" --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.PrintName				= "Tiamat MK-3"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("TiamatMK3.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.Damage		= 100					-- Damage, in standard damage points.
SWEP.DamageType = bit.bor(DMG_AIRBOAT,DMG_BLAST,DMG_ALWAYSGIB) --See DMG enum.  This might be DMG_SHOCK, DMG_BURN, DMG_BULLET, etc.
...
SWEP.NZPaPReplacement 	= "tfa_cso_plasmagunexd"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
SWEP.Primary.ClipSize			= 70					-- This is the size of a clip
SWEP.Primary.DefaultClip			= 420				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.Primary.StaticRecoilFactor = 0.40 	--Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.
...
SWEP.ProjectileEntity = "plasmaexc" --Entity to shoot
SWEP.ProjectileVelocity = 6500 --Entity to shoot's velocity
SWEP.ProjectileModel = "models/items/ar2_grenade.mdl" --Entity to shoot's model

--[[VIEWMODEL]]--

SWEP.ViewModel			= "models/weapons/tfa_cso/c_plasmaexc.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_plasmagunexc.mdl" -- Weapon world model path
...
SWEP.Secondary.ScopeTable = {
	["ScopeMaterial"] =  Material("scope/plasmagunexc_scope.png", "smooth"),
...
SWEP.IronSightsPos = Vector(7.85, -6.874, -0.45)
...
SWEP.MuzzleFlashEffect = "cso_muz_plasgunexc" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
...
SWEP.Secondary.Sound 			= Sound("TiamatMK3.Fire3")
...
SWEP.Secondary.Delay = 3 --Delay in seconds

DEFINE_BASECLASS(SWEP.Base)

function SWEP:AltAttack()
if CurTime() >  self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Clip1() > 0 then
self:SendViewModelAnim( ACT_VM_SECONDARYATTACK )
timer.Simple(2, function()
if SERVER then
local bullet = {}
self:EmitSound("TiamatMK3.Fire3")
self.Owner:ViewPunch(Angle(-1,0,0))
self:TakePrimaryAmmo( 1 )
bullet.Num = self.Primary.NumberofShots
bullet.Src = self.Owner:GetShootPos()
bullet.Dir = self.Owner:GetAimVector()
bullet.Spread = Vector( 0 * self.Primary.Spread, 0 * self.Primary.Spread, 0 )
bullet.Tracer = 1
bullet.Force = self.Primary.Force
bullet.Damage = self.Secondary.Damage
bullet.AmmoType = self.Primary.Ammo
bullet.TracerName = "cso_tra_thunbolt"
...
phys:AddAngleVelocity( Vector( 0, math.Rand( 125, 250 ), 0 ) )
end
end
end)
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_plasmagunexc")

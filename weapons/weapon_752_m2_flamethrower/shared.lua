...
if ( CLIENT ) then
	...
	killicon.Add( "weapon_752_30cal", "HUD/syndod/killicons/m2", Color( 255, 80, 0, 255 ) )
end

SWEP.ViewModel					= "models/weapons/syndod/v_m2.mdl"
SWEP.WorldModel					= "models/weapons/syndod/w_m2.mdl"
...
function SWEP:Precache()
	...
	util.PrecacheSound("ambient/machines/keyboard2_clicks.ogg")

	util.PrecacheSound("ambient/machines/thumper_dust.ogg")
	util.PrecacheSound("ambient/fire/mtov_flame2.ogg")
	util.PrecacheSound("ambient/fire/ignite.ogg")

	util.PrecacheSound("vehicles/tank_readyfire1.ogg")
...
function SWEP:Deploy()
	...
	if (SERVER) then
		self.Owner:EmitSound( "ambient/machines/keyboard2_clicks.ogg", 42, 100 )
	end
	return true
end
...
function SWEP:Think()
	if self.Owner:KeyReleased(IN_ATTACK) && (self.Owner:GetAmmoCount("ar2") > 1) && (self.ReloadDelay != 1) then
		if (SERVER) then
			self.Owner:EmitSound( "ambient/fire/mtov_flame2.ogg", 24, 100 )
		end
	end
	...
	if (self.Owner:GetAmmoCount("ar2") > 0) && (self.ReloadDelay == 0) then
		if self.Owner:KeyPressed(IN_ATTACK) then
			if (SERVER) then
				self.Owner:EmitSound( "ambient/machines/thumper_dust.ogg", 46, 100 )
			end
		end
		
		if self.Owner:KeyDown(IN_ATTACK) then
			if (SERVER) then
				self.Owner:EmitSound( "ambient/fire/mtov_flame2.ogg", math.random(27,35), math.random(32,152) )
			end
			local trace = self.Owner:GetEyeTrace()
			if (SERVER) then
...
function SWEP:Reload()
	...
	self.ReloadDelay = 1
	if (SERVER) then
		self.Owner:EmitSound( "vehicles/tank_readyfire1.ogg", 30, 100 )
	end
...
function SWEP:RunoutReload()
	...
	self.ReloadDelay = 1
	if (SERVER) then
		self.Owner:EmitSound( "ambient/machines/thumper_dust.ogg", 48, 100 )
		self.Owner:EmitSound( "vehicles/tank_readyfire1.ogg", 30, 100 )
	end
	timer.Simple(1.82, function() if self:IsValid() then self:ReloadSelf() end end)
end

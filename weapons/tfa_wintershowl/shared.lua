...
function SWEP:PreDrawViewModel( vm )
	if self.Ispackapunched then
		vm:SetSubMaterial(0, "models/weapons/common/bo1_pap_camo_large_c.vtf")
	else
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/freezegun/v_freezegun.mdl" --Viewmodel path
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/freezegun/w_freezegun.mdl" -- Weapon world model path
...
function SWEP:PrimaryAttack()
	...
	local shootpos, aimvec = own:GetShootPos(), own:GetAimVector()
	
	self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
	own:SetAnimation(PLAYER_ATTACK1)
	self:EmitSound("weapons/wintershowl/freeze_shot.ogg")
	self:SetNextPrimaryFire(CurTime() + self.Primary.Delay) -- Just regular SWEP stuff
	self:TakePrimaryAmmo(1)
	own:ViewPunch(Angle( math.Rand(-self.Primary.KickHorizontal, self.Primary.KickHorizontal), math.Rand(-self.Primary.KickUp, self.Primary.KickDown), 0))
	
	if SERVER then
		local orb1 = ents.Create("obj_whowl_proj") -- Launch the projectile. Purely visual.
		orb1:SetPos(shootpos)
		orb1:SetAngles(own:EyeAngles())
		orb1:Spawn()
	...
	dmg:SetDamageType(bit.bor(DMG_VEHICLE, DMG_REMOVENORAGDOLL))
	dmg:SetAttacker(own)
	...
	for k, v in pairs(ents.FindInCone(shootpos, aimvec, 512, 0.9)) do 
		tr.endpos = v:WorldSpaceCenter()
		local tr1 = util.TraceLine(tr)
		if v:IsPlayer() and !GetConVar("wintershowl_freezeplayers"):GetBool() then continue end
		if v:GetNWBool("icestaffed") or tr.HitWorld then continue end -- Checks if there's anything that should shield the target from us
		--if v == own then return end
		
		dmg:SetDamage(math.Clamp(v:Health()-1, 1, 1000)) -- Do damage, but don't kill it just yet
		if SERVER then
			v:TakeDamageInfo(dmg)
		end
		if !(v:IsPlayer() || v:IsNPC() || v:IsNextBot()) then continue end
		v:SetPlaybackRate( 0.5 ) -- Slow them down
		v:SetMaterial("effects/freeze_overlayeffect01")
		if v:Health() == 1 then -- They're doomed
			
			if v:IsNPC() then
				v:SetSchedule(SCHED_NPC_FREEZE) -- freeze NPCs
			elseif v:IsNextBot() then
				v.loco:SetDesiredSpeed(0)
				if string.find(v:GetClass(), "nz_zombie") then -- Freeze nextbots, especially zombies.
					v:SetBlockAttack(true)
				end
			elseif v:IsPlayer() then -- Freze players, if enabled.
				v:Freeze(true)
			end
			v:SetNWBool("icestaffed", true) -- We'll need this later
			v:EmitSound("weapons/wintershowl/freeze"..math.random(0,2)..".ogg")
			timer.Simple(math.Rand(4,6), function()
				if IsValid(v) then 
					if v:IsNPC() then
						v:SetSchedule(SCHED_NONE)
						--v:SetCondition(COND_NPC_UNFREEZE or 68)
					end
					if v:IsPlayer() then
						v:Freeze(false)
					end
					--v:EmitSound("weapons/wintershowl/shatter"..math.random(0,1)..".ogg")
					v:TakeDamageInfo(dmg)  -- Put them out of their misery
				end
			end)
			
		else -- They're still alive, slowly thawing
			
			if timer.Exists(v:EntIndex().."defrost") then
				timer.Stop(v:EntIndex().."defrost")
				timer.Start(v:EntIndex().."defrost")
			else
				timer.Create(v:EntIndex().."defrost", 8, 1, function()
...
function SWEP:PrimaryAttack()
	if not self:CanPrimaryAttack() or self.Owner:WaterLevel() > 2 then 
		self.Weapon:EmitSound("weapons/clipempty_rifle.ogg")
		self.Weapon:SendWeaponAnim(ACT_VM_PRIMARYATTACK_EMPTY)
		self:SetNextPrimaryFire(CurTime() + 0.25)
		return 
	end
	self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
	self.Weapon:EmitSound("weapons/freeze_shot.ogg")
	...
	local pos = self.Owner:GetShootPos() + Vector(0,0,0)
		...
		orb1:Spawn()
	--ParticleEffect("originstaff_ice_muzzle",o:GetShootPos()+o:GetAimVector()*64, o:EyeAngles(),o)
end
	self:TakePrimaryAmmo(1)
	if SERVER then
		local filter = {o}
		for i = 0, 11 do
			...
			if ent != nil and IsValid(ent) then
				...
				if ent:GetClass() == "nz_zombie_boss_panzer" then
					...
					d:SetDamage( math.floor(ent:Health()*.50) )
				end
				d:SetAttacker( o )
				d:SetInflictor( self )
...
function SWEP:Think2()
   BaseClass.Think2(self)
		if (self.Owner:GetVelocity():Length2DSqr() > 7000) and !self:GetIronSights() and !(self:GetStatus() == TFA.Enum.STATUS_RELOADING) then
			CurVMPos = Lerp(0.1, CurVMPos, Vector(-1.5, 0, -2.1))
			CurVMAng = Lerp(0.2, CurVMAng, Vector(1, 0, -2))
			--self.BobScaleCustom = 1
		else
			CurVMPos = Lerp(0.1, CurVMPos, Vector(1, 0, -0.2))
			CurVMAng = Lerp(0.2, CurVMAng, Vector(0, 0, 0))
		end

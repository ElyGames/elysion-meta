...
function SWEP:Deploy()
	...
	self:EmitSound("weapons/vfirethrower/deploy.ogg", 80, math.random(80, 120))
	...
	if IsFirstTimePredicted() then
		...
		if SERVER then
			...
			if (self.Owner:KeyPressed(IN_ATTACK) || !self.Sound) then
				...
				self.Sound = CreateSound(self.Owner, Sound("weapons/vfirethrower/fire.ogg"))
			end
...
function SWEP:PlayCloseSound()
	self:EmitSound("weapons/vfirethrower/close.ogg", 80, math.random(90, 110))
end
...
function SWEP:Holster(weapon)
	...
	self:EmitSound("weapons/vfirethrower/undeploy.ogg", 80, math.random(90, 110))

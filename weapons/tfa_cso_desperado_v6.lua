...
SWEP.PrintName = "Python Desperado Expert" -- Weapon name (Shown on HUD)
...
SWEP.Revolver = true -- Am I a revolver?
...
--Recoil Related
SWEP.Primary.KickUp = 0.00 -- This is the maximum upwards recoil (rise)
SWEP.Primary.KickDown = 0.00 -- This is the maximum downwards recoil (skeet)
...
--[[VIEWMODEL]]
--
SWEP.ViewModel = "models/weapons/tfa_cso/c_desperado_v6.mdl" --Viewmodel path
...
--[[WORLDMODEL]]
--
SWEP.WorldModel = "models/weapons/tfa_cso/w_desperado_men_v6.mdl" -- Worldmodel path
SWEP.HoldType = "duel" -- This is how others view you carrying the weapon. Options include:
SWEP.WElements = {
	["woman"] = { type = "Model", model = "models/weapons/tfa_cso/w_desperado_women_v6.mdl", bone = "ValveBiped.Bip01_L_Hand", rel = "", pos = Vector(7.506, 1.2, 4.234), angle = Angle(0, 80, -5), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.MuzzleFlashEffect = "cso_muz_des_m" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
--Tracer Stuff
SWEP.Tracer = 0 --Bullet tracer.  TracerName overrides this.
SWEP.TracerName = "cso_tra_des_m" --Change to a string of your tracer name.  Can be custom.
...
SWEP.AnimationReplacements = {
	["male"] = {
		...
		["in"] = {
			...
			["value"] = "run_start_m" --Number for act, String/Number for sequence
		},
		["loop"] = {
			["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
			["value"] = "run_idle_m", --Number for act, String/Number for sequence
			["is_idle"] = true
		},
		["out"] = {
			["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
			["value"] = "run_end_m" --Number for act, String/Number for sequence
		}
	},
	["female"] = {
		["draw"] = {
		...
		["in"] = {
			["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
			["value"] = "run_start_w" --Number for act, String/Number for sequence
		},
		["loop"] = {
			["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
			["value"] = "run_idle_w", --Number for act, String/Number for sequence
			["is_idle"] = true
		},
		["out"] = {
			["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
			["value"] = "run_end_w" --Number for act, String/Number for sequence
		}
	}
}
...
function SWEP:SecondaryAttack( )
	if not ( TFA.Enum.ReadyStatus[self:GetStatus()] or self:GetStatus() == TFA.GetStatus("shooting") ) then return end
	local f = self:GetNW2Bool("female")
	self:SetNW2Bool("female", not f)
	if f then
		self:SendViewModelAnim( ACT_VM_UNDEPLOY )
	else
		self:SendViewModelAnim( ACT_VM_DEPLOY )
	end
	self:SetStatus(TFA.GetStatus("reloading"))
	self:SetStatusEnd( CurTime() + 0.18 )
end
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_desperado_v6")

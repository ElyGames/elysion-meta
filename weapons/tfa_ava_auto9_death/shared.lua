...
SWEP.Manufacturer = "OCP" --Gun Manufactrer (e.g. Hoeckler and Koch )
...
SWEP.PrintName = "Auto 9 Death" -- Weapon name (Shown on HUD)
...
--
--Firing related
SWEP.Primary.Sound = Sound("TFA_AVA_AUTO9.1") -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.RPM_Burst = 1200 -- RPM for burst fire, overrides semi.  This is in Rounds Per Minute / RPM
SWEP.Primary.BurstDelay = 0.25 -- Delay between bursts, leave nil to autocalculate
...
SWEP.Primary.SpreadRecovery = 2.5 --How much the spread recovers, per second.
--Range Related
SWEP.Primary.Range = 300 * 16 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
SWEP.MoveSpeed = 0.975 --Multiply the player's movespeed by this.
...
--[[VIEWMODEL]]
--
SWEP.ViewModel = "models/weapons/tfa_ava/c_auto9.mdl" --Viewmodel path
...
--[[WORLDMODEL]]
--
SWEP.WorldModel = "models/weapons/tfa_ava/w_auto9.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-8.921, -5.159, 2.48)
SWEP.IronSightsAng = Vector(0.486, 0, 0)
...
SWEP.BlowbackBoneMods = {
	["Bone05"] = { scale = Vector(1, 1, 1), pos = Vector(0, 4.311, 0), angle = Angle(0, 0, 0) }
}
...
SWEP.MuzzleFlashEnabled = true --Do a muzzle flash?
	...
	[ACT_VM_DRAW] = {
		{
			['time'] = 0.33,
			['type'] = "sound",
			['value'] = Sound("TFA_AVA_AUTO9.Slide")
		}
	},
	[ACT_VM_RELOAD] = {
		{
			['time'] = 10 / 30,
			['type'] = "sound",
			['value'] = Sound("TFA_AVA_AUTO9.MagOut")
		},
		{
			['time'] = 36 / 30,
			['type'] = "sound",
			['value'] = Sound("TFA_AVA_AUTO9.MagIn")
		},
		{
			['time'] = 72 / 30 - 0.18,
			['type'] = "sound",
			['value'] = Sound("TFA_AVA_AUTO9.SlideReload")
		}
	}
}
...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Bip01_L_Finger0"] = { scale = Vector(1, 1, 1), pos = Vector(-2.225, -1.06, 0), angle = Angle(6.436, 4.804, 27.576) }
}
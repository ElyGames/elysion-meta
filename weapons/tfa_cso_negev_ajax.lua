...
SWEP.PrintName				= "Negev NG-7 Ajax"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("NG_Ajax.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("NG_Ajax.Fire2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage_Regular		= 85					-- Damage, in standard damage points.
SWEP.Primary.Damage		= SWEP.Primary.Damage_Regular					-- Damage, in standard damage points.
SWEP.Primary.Damage_Secondary		= 85 * 3					-- Damage, in standard damage points.
SWEP.Primary.BlastRadius	= 25               -- Radius for DMG_BLAST
...
SWEP.Primary.DefaultClip			= 600				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_negev_ajax.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_negev_ajax.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "cso_muz_ajax" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_ajax"	--Change to a string of your tracer name.  Can be custom.
...
SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_negev_ajax")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.TracerName = "cso_tra_ajax"
			self.Primary_TFA.RPM = 750
			self.Primary_TFA.Spread = .005
			self.Primary_TFA.AmmoConsumption = 1
			self.Primary_TFA.NumShots = 1
			self:ClearStatCache()
		else
			self.TracerName = "tra_ajax"
			self.Primary_TFA.Damage = self.Primary_TFA.Damage_Secondary
			self.Primary_TFA.RPM = 240
			self.Primary_TFA.Spread = .0025
			self.Primary_TFA.AmmoConsumption = 3

...
SWEP.PrintName				= "Shining Heart Rod"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("MagicRod.Fire")				-- This is the sound of the weapon, when you shoot.
...
SWEP.ProjectileEntity = "magic_roda" --Entity to shoot
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_magic_rod.mdl" --Viewmodel path
...
SWEP.VElements = {
    ["draw_effect"] = { type = "Model", model = "models/hunter/plates/plate1x1.mdl", bone = "Dummy003", rel = "", pos = Vector(7, 7, 10), angle = Angle(-20, 128, 0), size = Vector(0.75, 0.75, 0), color = Color(255, 255, 255, 255), surpresslightning = false, material = "sprites/ef_magicmgdraw", skin = 0, bodygroup = {}, active = false}
}

SWEP.EventTable = {
    [ACT_VM_DRAW] = {
        {time = 1 / 30, type = "lua", value = function(wep)
            wep.VElements.draw_effect.active = true
            wep:ClearStatCache("VElements.draw_effect.active")
        end}
    }
}
--[[WORLDMODEL]]--

SWEP.WorldModel			= "models/weapons/tfa_cso/w_magic_rod.mdl" -- Weapon world model path
...
SWEP.Secondary.ScopeTable = {
	["ScopeMaterial"] =  Material("scope/at4/at4_scope.png", "smooth"),
...
SWEP.Secondary.Ent = "magic_rodb"

SWEP.Secondary.Model = "models/hunter/misc/sphere1x1.mdl" --Projectile Model
SWEP.Secondary.Damage = 2500 --Grenade Damage
SWEP.Secondary.Velocity = 1500 --Grenade Velocity
SWEP.Secondary.Delay = 2 --Delay in seconds
SWEP.Secondary.Sound = Sound("MagicRod.Fire2") --Firing sound
SWEP.Secondary.Recoil = Angle(0,0,0) --Recoil Dir

SWEP.Base				= "tfa_gun_base"
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_magic_rod")
	SWEP.DrawWeaponSelection = TFA_CSO_DrawWeaponSelection
end

DEFINE_BASECLASS(SWEP.Base)
function SWEP:Holster( ... )
	self:StopSound("MagicRod.Idle")
...
if SERVER then
if self.NextGen >= 30 and self:Ammo2() < 1 then
self.Owner:GiveAmmo( 1,self.Secondary.Ammo, true )   
self:EmitSound("MagicRod.Alarm" )    
self.NextGen  = 0
end

...
SWEP.Author				= "Raven & Roach" --Author Tooltip
SWEP.Contact				= "Steam" --Contact Info Tooltip
SWEP.Purpose				= "Turn zombies into CIA agents that will explode, great for baiting zombies." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.DrawCrosshairIS = false --Draw the crosshair in ironsights?
SWEP.PrintName				= "V-R11"		-- Weapon name (Shown on HUD)
...
--Firing related
SWEP.Primary.Sound 			= Sound("Weapon_VR11.Shoot")				-- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.Ammo			= "GaussEnergy"					-- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
...
SWEP.ProjectileEntity = "obj_humangun_proj" --Entity to shoot
SWEP.ProjectileVelocity = 2000 --Entity to shoot's velocity
SWEP.ProjectileModel = nil --Entity to shoot's model

--[[VIEWMODEL]]--

SWEP.ViewModel			= "models/weapons/vr11/v_vr11.mdl" --Viewmodel path
...
SWEP.VMPos = Vector(1.004, -1.407, -2.01)
SWEP.VMAng = Vector(7.309, 2.809, 0)
...
SWEP.WorldModel			= "models/weapons/vr11/w_vr11.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-4.915, 0.079, 2.3)
SWEP.IronSightsAng = Vector(-6.95, -2.852, 0)
...
function SWEP:PrimaryAttack()
	...
	own:ViewPunch(Angle(-2,-0.1,0))
	
	if SERVER then
		local proj = ents.Create("obj_humangun_proj")		
		proj:SetPos(own:GetShootPos())
		proj:SetOwner(own)		
		proj:SetUpgraded(self.Ispackapunched)
		proj:Spawn()
		proj:Activate()
	...
	if self.Ispackapunched then
		ParticleEffectAttach("vr11_muzzle_pap", PATTACH_POINT_FOLLOW, own:GetViewModel(), 1)
	else
		ParticleEffectAttach("vr11_muzzle", PATTACH_POINT_FOLLOW, own:GetViewModel(), 1)
	end
...
function SWEP:PreDrawViewModel( vm )
	if self.Ispackapunched then
		...
		--vm:SetSubMaterial(1, "models/weapons/common/bo1_pap_camo_c.vmt")
	else
